...
DEF=`ip route show table $table|grep default|grep $ap_int`
  if ! [ -z "$DEF" ]; then
     break
  fi
done
...
iptables --table nat --append POSTROUTING --out-interface $ap_int -j MASQUERADE
...
ip rule add from all iif lo oif $ap_int lookup $table pref 17000 2> /dev/null
